...
from system_tools.errors import DependenciesException
...
class BaseTestPlatform:
    ...
    def check_system_setup(self):
        ...
        if not self._is_virtualization():
            raise DependenciesException("Virtualization is not setting properly")
        if not self._is_kvm():
            raise DependenciesException("KVM is not setting properly")
        if not self.pms:
            raise DependenciesException("Packet manager is not setting properly")
        if not self._is_qemu():
            raise DependenciesException("QUEMU is not setting properly")
        if not self._set_security_policies():
            raise DependenciesException("Security polices is not setting properly")
        if not self._is_docker():
            raise DependenciesException("Docker is not setting properly")
